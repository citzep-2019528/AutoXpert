...
public class EmpleadoDAO {
    ...
    public int agregar (Empleado emp){
        String sql = "Insert into Empleado (DPIEmpleado, primerNombre, otrosNombres, apellidos, telefonoEmpleado, direccionEmpleado, estadoEmpleado, usuario, codigoSucursal, codigoTipoEmpleado) values (?,?,?,?,?,?,?,?,?,?)";
        try{
            con = cn.Conexion();
            ps = con.prepareCall(sql);
            ...
            ps.setInt(9, emp.getCodigoSucursal());
            ps.setInt(10, emp.getCodigoTipoEmpleado());
            ps.executeUpdate();
        }catch(Exception e){
            e.printStackTrace();
        }
    ...
    public int actualizar(Empleado emp){
        String sql = "Update Empleado set DPIEmpleado =?, primerNombre =?, otrosNombres =?, apellidos =?, telefonoEmpleado =?, direccionEmpleado =?, estadoEmpleado=?, usuario=?" + "where codigoEmpleado = ?";
        try{
            con = cn.Conexion();
            ps  = con.prepareCall(sql);
            ps.setString(1, emp.getDPIEmpleado());
